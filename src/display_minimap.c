...
/*   Updated: 2022/08/26 18:56:32 by alefranc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_minimap(t_all *all, t_data *img)
{
	...
	while (y < ft_strtabsize(all->map))
	{
		...
		{
			if (all->map[y][x] == '1')
				my_mlx_square(img, x * 8, y * 8, 0x00FF0000);
			if (all->map[y][x] == '0')
				my_mlx_square(img, x * 8, y * 8, 0x0000FF00);
			if (all->map[y][x] == 'N' || all->map[y][x] == 'S' ||
					all->map[y][x] == 'E' || all->map[y][x] == 'W')
				my_mlx_square(img, x * 8, y * 8, 0x000000FF);
			x++;
		}
		y++;
	}
...
int	display_minimap(t_all *all, int offsetx, int offsety)
{
	...
	img.img = mlx_new_image(all->mlx, ft_strlen(all->map[0]) * 8,
			ft_strtabsize(all->map) * 8);
